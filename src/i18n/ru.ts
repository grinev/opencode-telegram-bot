...
export const ru: I18nDictionary = {
  ...
  "prompt_queue.queued": "⏳ В очереди (#{position}) — запустится, когда агент освободится.",
  "prompt_queue.running": "▶️ Выполняю это сообщение из очереди.",
  "prompt_queue.cancelled": "🚫 Сообщение из очереди отменено.",
  "prompt_queue.cancel_too_late": "Уже запущено — отменить нельзя.",
  "prompt_queue.dropped": "🚫 Сообщение из очереди отброшено: сессия изменилась.",
  "prompt_queue.cleared": "🚫 Очередь очищена: отброшено сообщений — {count}.",
  "cmd.description.unqueue": "Удалить последнее сообщение из очереди",
  "unqueue.empty": "📭 Нет сообщений в очереди для удаления.",
  "unqueue.removed": "↩️ Последнее сообщение из очереди удалено (осталось {remaining}):\n{preview}",
  "prompt_queue.failed": "🔴 Не удалось выполнить сообщение из очереди.",
  "prompt_queue.full": "⏳ Очередь заполнена ({limit}). Дождитесь агента или используйте /abort.",
  "prompt_queue.button.cancel": "❌ Отмена",
};
